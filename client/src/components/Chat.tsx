...
const Chat = ({id, title, connectedUsers} : IChat) => {
  ...
  const handleClick = () => {
    ...
    const element = document.getElementById(`chat-${id.toString()}`) as HTMLElement
  ...
  const handleMouseOver = () => {
    const hoveredElement = document.getElementById(`chat-${id.toString()}`);
  ...
  return (
    <article id={`chat-${id.toString()}`} onClick={handleClick} onMouseOver={handleMouseOver} onMouseLeave={handleMouseLeave} className="flex min-h-[4rem] items-center px-4 py-2 cursor-pointer gap-4">
